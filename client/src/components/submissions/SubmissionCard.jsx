...
import { CheckCircle, XCircle, Clock, ExternalLink, MessageSquare, User } from "lucide-react"
...
export default function SubmissionCard({ submission, userRole }) {
  ...
  const { color, icon: StatusIcon, text: statusText } = getStatusInfo(submission.status)
      ...
      <CardHeader className="pb-4">
        ...
        <CardDescription className="text-gray-400 flex items-center gap-2">
          ...
          <span className="font-medium">
            Submitted by: {submission.student ? submission.student.email : "Unknown Student"}
          </span>
        </CardDescription>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between border-b border-gray-700 pb-4">
          <div className="flex items-center gap-2">
            <StatusIcon className={`h-5 w-5 ${color.replace("bg-", "text-")}`} />
            <Badge className={`${color} text-white text-base px-3 py-1 rounded-full`}>{statusText}</Badge>
          </div>
          <a
        ...
        {submission.note && (
          <div className="bg-gray-700 p-4 rounded-lg shadow-inner border border-gray-600">
            <p className="font-semibold text-gray-300 flex items-center gap-2 mb-2">
              <MessageSquare className="h-5 w-5 text-purple-300" /> Student Note:
            </p>
            <p className="text-gray-400 italic leading-relaxed">{submission.note}</p>
          </div>
        )}
        {submission.feedback && (
          <div className="bg-gray-700 p-4 rounded-lg shadow-inner border border-gray-600">
            <p className="font-semibold text-gray-300 flex items-center gap-2 mb-2">
              <MessageSquare className="h-5 w-5 text-purple-300" /> Instructor Feedback:
            </p>
            <p className="text-gray-400 leading-relaxed">{submission.feedback}</p>
          </div>
        )}
        {isInstructor && (
          <div className="flex justify-end pt-4">
            <Link href={`/dashboard/submissions/${submission._id}`}>
              <Button className="bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-lg px-6 py-3 shadow-md hover:shadow-lg transition-all duration-300">
                Review Submission
              </Button>
            </Link>
          </div>
